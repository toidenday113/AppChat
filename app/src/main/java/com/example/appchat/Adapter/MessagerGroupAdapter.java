    ...
    private void DeleteAllMessenger(){
        ...
        builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
            ...
            public void onClick(final DialogInterface dialog, int which) {
                for(int i =0; i <arrMessegeGroup.size(); i++){
                    DatabaseReference RefDelete = FirebaseDatabase.getInstance().getReference("Groups").child(idGroup).child("Messengers").child(arrMessegeGroup.get(i).getId());
                    RefDelete.removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task) {
                            if(task.isSuccessful()){
