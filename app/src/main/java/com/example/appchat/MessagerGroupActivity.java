...
public class MessagerGroupActivity extends AppCompatActivity {
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        MenuItem register = menu.findItem(R.id.menu_delete_group);
        if(fUser.getUid().equals(itemGroup.getAdmin())){
            register.setVisible(!isRestricted());
        }
        return true;
    }
    ...
    private void DeleteOneGroup(){
        ...
        input.setText("This group will be deleted");
        builder.setView(input);
        ...
        builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
            ...
            public void onClick(final DialogInterface dialog, int which) {
                ...
                DatabaseReference RefDeleteGroup = FirebaseDatabase.getInstance().getReference("Groups").child(itemGroup.getId());
                RefDeleteGroup.removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                            if(task.isSuccessful()){
                                RemoveUserInGroup();
    ...
    private  void RemoveUserInGroup(){
        DatabaseReference RefRemoveUserInGroup = FirebaseDatabase.getInstance().getReference("Users");
        RefRemoveUserInGroup.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for(DataSnapshot dataSnapshot : snapshot.getChildren()){
                    final String KeyUser = dataSnapshot.getKey();
                    DatabaseReference RefmGroup = FirebaseDatabase.getInstance().getReference("Users").child(KeyUser).child("mGroup");
                    RefmGroup.addValueEventListener(new ValueEventListener() {
                        @Override
                        public void onDataChange(@NonNull DataSnapshot snapshot1) {
                            for(DataSnapshot dataSnapshot1 : snapshot1.getChildren()){
                                String Key  = dataSnapshot1.getKey();
                                String idGroup = dataSnapshot1.child("idGroup").getValue().toString();
                                if(itemGroup.getId().equals(idGroup)){
                                    DatabaseReference RefRemove = FirebaseDatabase.getInstance().getReference("Users").child(KeyUser).child("mGroup").child(Key);
    ...
    private void JoinGroup(){
        UserJoinGroupBottomSheetDialog userJoinGroupBottomSheetDialog = new UserJoinGroupBottomSheetDialog();
        userJoinGroupBottomSheetDialog.getIdGroup(itemGroup.getId());
        userJoinGroupBottomSheetDialog.show(getSupportFragmentManager(),"BottomSheet");
    }
